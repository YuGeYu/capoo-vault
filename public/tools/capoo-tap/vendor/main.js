...
function openToolsList() {
  window.location.assign('/tools/list');
}
...
profileButton.addEventListener('click', openToolsList);
...
async function start() {
  ...
  try {
    ...
    started = false;
    pointers.clear();
    subEl.textContent = '声音加载失败，点击重试';
    overlay.classList.remove('hide');
    showControls();
    console.warn('[咖波节奏拍] 音频初始化失败，可再次点击重试。', error);
  }
}
