...
async function queryD1(sql) { const { stdout } = await execFileAsync('npx', ['wrangler', 'd1', 'execute', 'MMC_DB', '--remote', '-c', 'wrangler.production.toml', '--json', `--command=${sql}`], { maxBuffer: 20 * 1024 * 1024 }); const start = stdout.indexOf('[{'); if (start < 0) throw new Error(`无法解析 D1 JSON: ${stdout}`); const parsed = JSON.parse(stdout.slice(start)); return parsed.flatMap(item => item.results || []); }
